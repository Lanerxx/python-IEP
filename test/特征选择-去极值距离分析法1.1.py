...
def quartileMethod(data):
    ...
    quartile2 = np.percentile(data, (50))
...
def laner_score(indexfeaturesCut):
    prediect = []
    # 测试每一项数据
    for row in range(0,sumDates):
        ...
        distant0 = 0
        for indexFeature in range(0,sumFeatures):
            if indexFeature != indexfeaturesCut:
                distant1 += abs(X[row][indexFeature] - aveFeatureOne_Zero[0][indexFeature])
                distant0 += abs(X[row][indexFeature] - aveFeatureOne_Zero[1][indexFeature])
...
# 所有特征综合
sumAffectFeature  = laner_score(-1)
# 删除单个特征
indexFeatures = 0
while indexFeatures < sumFeatures:
    print(features[indexFeatures])
    affectFeature = laner_score(indexFeatures)
